...
def probabilities(model, loader, device):
    model.eval();outputs={name:[] for name in ("fusion","vision","tabular","cross")};labels=[];gates=[]
    with torch.no_grad():
        for batch in loader:
...
def main():
    ...
    parser.add_argument("--out",type=Path,default=Path("models/multimodal_v2"))
    parser.add_argument("--checkpoint",type=Path,help="Local TerraMind checkpoint matching --variant; omit for automatic download")
    parser.add_argument("--tabular-checkpoint",type=Path,default=Path("models/statewide/feature_transformer.pt"))
    parser.add_argument("--tabular-preprocessing",type=Path,default=Path("models/statewide/transformer_preprocessing.joblib"))
    ...
    parser.add_argument("--gate-balance",type=float,default=.001);parser.add_argument("--patience",type=int,default=10)
    ...
    preprocessing=joblib.load(args.tabular_preprocessing)
    if preprocessing["features"]!=FEATURES:raise RuntimeError("Tabular preprocessing feature order mismatch")
    raw=frame[FEATURES].to_numpy(dtype=np.float32);imputed=preprocessing["imputer"].transform(raw).astype(np.float32)
    scaler=preprocessing["scaler"];tabular=scaler.transform(imputed).astype(np.float32)
    ...
    tabular_expert=FeatureTransformer(len(FEATURES),dimension=32,n_classes=4)
    tabular_expert.load_state_dict(torch.load(args.tabular_checkpoint,map_location="cpu",weights_only=True))
    for parameter in tabular_expert.parameters():parameter.requires_grad=False
    model=MultimodalFusionModel(backbone,len(FEATURES),vision_dim,tabular_expert=tabular_expert).to(device)
    ...
    best_score=-1;best_state=None;best_epoch=0;patience=0;history=[];trainable_blocks=[]
    for epoch in range(1,args.epochs+1):
        ...
        model.train();model.tabular.eval();optimizer.zero_grad(set_to_none=True);losses=[]
        for step,batch in enumerate(loaders["train"],1):
            result=model(batch["chips"].to(device),batch["tabular"].to(device));target=batch["label"].to(device)
            mean_gate=result["gate"].mean(dim=0)
            gate_balance=(mean_gate*torch.log(mean_gate.clamp_min(1e-8))).sum()
            loss=criterion(result["fusion"],target)+.25*criterion(result["vision"],target)+.25*criterion(result["cross"],target)+args.gate_balance*gate_balance
        ...
        if score>best_score+1e-4:best_score=score;best_epoch=epoch;best_state=copy.deepcopy(model.state_dict());patience=0
        else:patience+=1
        if patience>=args.patience:
            print(f"[early-stop] no validation macro-F1 improvement for {args.patience} epochs; restoring epoch {best_epoch}",flush=True)
            break
    ...
    checkpoint={"state_dict":model.state_dict(),"features":FEATURES,"tabular_mean":scaler.mean_.tolist(),"tabular_scale":scaler.scale_.tolist(),"vision_dim":vision_dim,"bands":S2_BANDS,"seasons":["dry_Jan_Mar","monsoon_Jun_Sep"],"weak_label_version":VERSION,"training_districts":sorted(frame.loc[train_mask,"district"].unique()),"model":f"TerraMind-1.0-{args.variant} + pretrained tabular expert + normalized logit mixture","variant":args.variant,"fusion_version":2,"trainable_backbone_parameters":trainable_blocks}
    torch.save(checkpoint,args.out/"terramind_tabular_fusion.pt")
    report={"dataset_sha256":hashlib.sha256(args.data.read_bytes()).hexdigest(),"chip_count":len(frame),"chip_shape":[2,12,224,224],"split":"Same whole-district holdout as statewide tabular study","validation_districts":VALIDATION_DISTRICTS,"test_districts":TEST_DISTRICTS,"results":results,"best_validation_macro_f1":best_score,"best_epoch":best_epoch,"epochs_run":len(history),"device":str(device),"fusion_version":2,"target_warning":"Experimental weak labels, not observed construction outcomes"}
